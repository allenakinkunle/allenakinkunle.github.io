...
### Motivation
In supervised machine learning, cost functions are used to measure a trained model's performance on data. The most commonly used cost function for regression is the Mean Squared Error (MSE), and for binary classification, Cross Entropy is the most common choice. For many people, the reasons for choosing these cost functions are not at all clear.  
...
The likelihood measures how plausible it is that the observed data was generated by the model with a particular value of $\theta$.
